...
module.exports = {
    ...
    const mappedLeaderboard = leaderboard.map((i, p = 0)=> {
      ...
      return `\`\`${p+1}. ${`\`\`**${bot.users.cache.get(i.userId)}**\`\`#${bot.users.cache.get(i.userId)}\`\`` ? `\`\`**${bot.users.cache.get(i.userId).username}**\`\`#${bot.users.cache.get(i.userId).discriminator}\`\`` : `**Неизвестный**\`\`#0000\`\``} — \`\`${i.coinsInBank}\`\` <a:danndollar:875448360830644225>`});
