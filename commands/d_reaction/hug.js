...
module.exports = {
  ...
  run: async (bot, message, args) => {
    try {
        ...
        if(!args[0]) return message.channel.send(embed.setDescription("Укажи участника чтобы обнять его/ее.")).then(msg => {msg.delete({timeout: "10000"})});
        ...
        if (!member) return message.channel.send(embed.setDescription("Укажи участника чтобы обнять его/ее.")).then(msg => {msg.delete({timeout: "10000"})});
        if (member.id === message.author.id) return message.channel.send(embed.setDescription('Ты не сможешь обнять себя.')).then(msg => {msg.delete({timeout: "10000"})});
