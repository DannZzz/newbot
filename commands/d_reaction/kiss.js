...
module.exports = {
  ...
  run: async (bot, message, args) => {
    try {
        ...
        if(!args[0]) return embed(message).setError("Укажи участника чтобы поцеловать его/ее.").send().then(msg => {msg.delete({timeout: "10000"})});
        ...
        if (!member) return embed(message).setError("Укажи участника чтобы поцеловать его/ее.").send().then(msg => {msg.delete({timeout: "10000"})});
        if (member.id === message.author.id) return embed(message).setError('Ты не сможешь поцеловать себя.').send().then(msg => {msg.delete({timeout: "10000"})});
