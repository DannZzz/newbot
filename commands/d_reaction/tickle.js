...
module.exports = {
  ...
  run: async (bot, message, args) => {
    try {
        ...
        if(!args[0]) return message.channel.send(embed.setDescription("Укажи участника чтобы пощекотать его/ее.")).then(msg => {msg.delete({timeout: "10000"})});
        ...
        if (!member) return message.channel.send(embed.setDescription("Укажи участника хчтобы пощекотать его/ее.")).then(msg => {msg.delete({timeout: "10000"})});
        if (member.id === message.author.id) return message.channel.send(embed.setDescription('Ты не сможешь пощекотать себя.')).then(msg => {msg.delete({timeout: "10000"})});
